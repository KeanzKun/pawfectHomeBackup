...
import { Alert, Text, BackHandler, TouchableOpacity, StyleSheet, Pressable, View, ScrollView, TouchableHighlight, Dimensions } from "react-native";
...
const DeleteAccount2 = () => {
  ...
  const deleteUserAccount = async () => {
    try {
      const response = await fetch(`${SERVER_ADDRESS}/api/users/${userDetails.user.userID}`, {
        method: 'DELETE',
        headers: {
  ...
  useEffect(() => {
    ...
    const handleBackPress = () => {
      navigation.navigate('AccountSetting'); // Exit the app
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.login}>
        <View style={styles.welcomeContainer}>
          ...
          <Text style={styles.loginToContinueText}>Once the action is done, all your data will be deleted from our database, and it cannot be recovered back.</Text>
          <Text style={[styles.loginToContinueText, { color: Color.sandybrown }]}>This action cannot be undone.</Text>
        </View>

        <View style={styles.redirectSignUp}>
          <TouchableHighlight //Cancel
            style={styles.cancelButton}
            onPress={() => navigation.goBack()}
            underlayColor={Color.sandybrown}
          >
            <Text style={styles.cancelButtonText}>NO, Cancel and Back</Text>
          </TouchableHighlight>


          <TouchableHighlight //Delete Account
            style={styles.button}
            onPress={deleteUserAccount} // Call the deleteUserAccount function when pressed
            underlayColor={Color.sandybrown}
          >
            <Text style={styles.buttonText}>Understood, Continue</Text>
          </TouchableHighlight>
